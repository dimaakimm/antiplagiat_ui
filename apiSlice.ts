...
const baseQuery = fetchBaseQuery({
    ...
    prepareHeaders: (headers) => {
        ...
        if (!uuid) {
            // Генерируем новый UUID если его нет в localStorage
...
> = async (args, api, extraOptions) => {
    ...
    //     const refreshResult = await baseQuery('/refresh', api, extraOptions) //TODO разобраться с беком что точно будет приходить при данном запросе
